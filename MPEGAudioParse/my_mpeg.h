...
namespace mpeg {
    ...
    class buffer : public my::io::buffer_guts_type<buffer<READER>> {
        ...
        READER m_cb;
        template <typename T> buffer(T& reader) : base(*this), m_cb(reader) {}

        public:
        static auto make_buffer(READER&& reader) { return buffer<READER>(reader); }
        buffer(READER&& reader) : buffer(reader) {}

        int get(int& how_much, bool peek, seek_type sk) {
    ...
    template <typename CB> using buffer_t = buffer<CB>;
    struct error {
...
parser(int64_t file_size = 0) : file_size(file_size = -1) {}
